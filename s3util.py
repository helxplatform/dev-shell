...
def get_session(values):
   ...
   s3 = session.resource(service_name='s3',endpoint_url=values['host'])
   bucket = s3.Bucket(values['bucket'])
   return s3,bucket
...
def main(args):
   ...
   if values != None:
      try:
         s3_handle,bucket = get_session(values)
         if args.command == 'ls':
            for obj in bucket.objects.all(): print(obj.key)
         elif args.command == 'get':
            obj = s3_handle.Object(bucket.name,args.object)
            response = obj.get()
            data = response['Body'].read()
            filename = args.object.split('/')[-1]
            with open(filename,"wb") as output_file: output_file.write(data)
         elif args.command == 'put':
            filename = args.src.split('/')[-1]
            prefix = args.dst
            if prefix == None: prefix = ''
            obj = s3_handle.Object(bucket.name,prefix+filename)
